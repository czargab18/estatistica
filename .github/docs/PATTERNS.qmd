...
- `dev*`: branch "simula" o ambiente de **`desenvolvimento`**. para cada "*" é um possível desenvolvedor que pode contribuir. Além diso podemos ter branchs dessesdesenvolvedores com: `dev1renbr` que é a branch de ``renderização de books`` do ``dev1``
