...
def codigo():
    ...
    if padrao:
        ...
        if pergunta(opcao="t", pergunta="Deseja tentar novamente"):
            return codigo()
        else:
...
def gen_identificador():
    ...
    while identificador_existe(identificador):
        identificador = "".join(random.choice(characters)
...
def save_article_to_html(article_info, template_html, output_filepath):
    ...
    def update_section(section_name, content):
        section_meta = soup.find(
            'meta', {'data-section-article': section_name})
        if section_meta:
            section_div = section_meta.find_next_sibling(
                'div', class_='pagebody-copy')
            if section_div:
                section_div.clear()
...
def up_article_json(artigo=None, path="/newsroom/posts/data/articles.json"):
    if artigo is None:
        ...
        if pergunta("t", "Deseja executar função: conteudo()"):
            artigo = conteudo()
