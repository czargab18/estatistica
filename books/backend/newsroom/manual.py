...
def identificador(caminho: str = "./config/data/articles.json",
    ...
    Args:
        caminho (str): O caminho do arquivo JSON que contém os identificadores.
    """
    def gerar_identificador():
        LENGTH = 10
        return ''.join(random.choices(string.ascii_lowercase + string.digits, k=LENGTH))

    if identificador is True:
        return identificador
    else:
        if caminho is None:
            caminho = "./config/data/articles.json"
        else:
            try:
                conteudo_json = import_content_file(caminho)
...
def meta_info(caminho: str = "./config/scripts/newsroom/article/artigo.txt"):
    """
    ...
    meta_info = {}
    conteudo = import_content_file(caminho)
...
def content_article(caminho:str = "./config/scripts/newsroom/article/artigo.txt"):
    """
...
def save_content_article_in_json(caminho: str = "./config/data/article.json"):
    """
    Salva o conteúdo do artigo em um arquivo JSON.
    O arquivo JSON é salvo em ././config/data/article.json
    """
    if caminho is None:
        caminho = "./config/data/article.json"
...
def template_html(
    caminho_modelo: str = "./config/scripts/newsroom/modelo/modelo.html",
    caminho_json: str = "./config/data/article.json"
    ):
    if caminho_modelo is None:
        caminho_modelo = "./config/scripts/newsroom/modelo/modelo.html"
...
def mover_html(
        caminho_json: str = "./config/data/article.json",
        caminho_html_atualizado: str = "./config/scripts/newsroom/article/index.html",
        caminho_src: str = "./config/scripts/newsroom/article/src/"):
    
    if caminho_json is None:
        caminho_json = "./config/data/article.json"
    if caminho_html_atualizado is None:
        caminho_html_atualizado = "./config/scripts/newsroom/article/index.html"
    if caminho_src is None:
